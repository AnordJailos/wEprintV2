    ...
    paths: {
      ...
      '/api/v1/ai/chat/stream': {
        post: {
          tags: ['ai'],
          summary:
            'Ask the studio assistant and receive the answer as a Server-Sent Events stream. ' +
            'Frames are `event: sources` ({sources:[…]}), repeated `event: delta` ({text:"…"}) ' +
            'and a final `event: done` ({answer, sources}); `event: error` signals failure.',
          requestBody: {
            required: true,
            content: {
              'application/json': {
