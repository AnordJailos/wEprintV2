...
export async function readJson<S extends ZodTypeAny>(req: Request, schema: S): Promise<ZodOutput<S>> {
  ...
  if (!result.success) {
    ...
    throw ApiError.badRequest(issue ? `${issue.path.join('.') || 'body'}: ${issue.message}` : 'Invalid body.');
  }
  return result.data;
}
...
export function readQuery<S extends ZodTypeAny>(req: Request, schema: S): ZodOutput<S> {
  ...
  if (!result.success) {
    const issue = result.error.issues[0];
    throw ApiError.badRequest(issue ? `${issue.path.join('.') || 'query'}: ${issue.message}` : 'Invalid query.');
  }
  return result.data;
}
